...
%    A{3} = rand(n,n);  %A{3} = triu(A{3});  
    ...
    A{2} = A{1};
    A{3} = A{1};
    xe   = ones(n^3,1);%rand(n^3,1);
    b = (               kron(A{3},eye(n^2))  ...
        + kron(eye(n  ),kron(A{2},eye(n  ))) ...
        + kron(eye(n^2),     A{1})         )*xe;

    x = KroneckerSumSolver(A,b,3);
%    xNew = KroneckerSumSolverNew(A,b,3);

    test3 = norm(x-xe);
    fprintf('The test for degree=2 has error %g\n',test3);
%    test3 = norm(xNew-xe);
%    fprintf('The new implementation has error %g\n',test3);
    
  case 4
    %  The degree=4 case...

    A{1} = rand(n,n);  A{1} = triu(A{1});  
    A{2} = A{1};%rand(n,n);  A{2} = triu(A{2});  
    A{3} = A{1};%rand(n,n);  A{3} = triu(A{3});  
    A{4} = A{1};%rand(n,n);  A{4} = triu(A{4});  
    xe   = ones(n^4,1);%rand(n^4,1);
    b = (               kron(A{4},eye(n^3))  ...
...
%    xNew = KroneckerSumSolverNew(A,b,4);
...
%    xNew = KroneckerSumSolverNew(A,b,5);
